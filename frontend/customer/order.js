...
async function fetchRestaurants(cuisineType)  {
  ...
  const existingRestaurants = Array.from(restaurantListDiv.children).map(el => el.textContent.trim());
...
const uniqueRestaurants = [...new Set(data.map(restaurant => restaurant.trim().toLowerCase()))]; // Trim and lowercase names
...
uniqueRestaurants.forEach(restaurant => {
  ...
  listItem.addEventListener('click', () => fetchDishesForRestaurant(restaurant)); // New event listener
 ...
 restaurantListDiv.addEventListener('click', (event) => {
  if (event.target.tagName === 'P') { // Check if the clicked element is a paragraph (list item)
...
async function fetchDishesForRestaurant(restaurantName) {
  ...
  try {
    const response = await fetch(`http://localhost:3000/api/restaurantDishes?restaurant=${restaurantName}`);
...
script.src = 'https://maps.googleapis.com/maps/api/js?key=AIzaSyCmuVWpm0lkdNiq3wWZpEz0XYtDjmoN-wY&callback=initMap&libraries=places&loading=async'; // Replace 'YOUR_API_KEY' with your actual Google Maps API key
script.async = true;
...
function placeOrder () {
  ...
  const categorySelectionType = document.getElementById('categoryDropdown')? 'dropdown' : 'list';
  const restaurantSelectionType = document.getElementById('restaurantDropdown')? 'dropdown' : 'list';
  ...
  if (restaurantSelectionType === 'dropdown') {
    ...
    selectedRestaurant = document.getElementById('selectedRestaurant').textContent;
  }
  ...
  if (!selectedCategory ||!selectedRestaurant) {
   ...
   console.log("Retrieving Selected Category:", document.getElementById('selectedCategory').textContent);
...
const orderedDishes = Array.from(cartItems).map(item => {
  ...
  function extract_full_price(dishNameAndPrice) {
    ...
    const priceStr = parts[1].trim(); // Remove leading/trailing whitespace
      ...
      console.error(`Error extracting price for dish: ${dishNameAndPrice}`, error);
      return null;
    }
  }
  ...
  const orderData = {
    customerName,
    phoneNumber,
    selectedCategory: document.getElementById('selectedCategory').textContent || document.getElementById('categoryDropdown').value,
    selectedRestaurant,
    customerLocation,
    expectedDeliveryTime,
  ...
  fetch('http://localhost:3000/api/orders', {
    ...
    .catch(error => {
      ...
      alert('Please check you order form and fill your details and place you order again.')
    })
  });
}
